...
const SLOT_RULES: SlotRule[] = [
  { slot: "email", patterns: [/e-?mail|mail address|メール|メールアドレス|邮箱|電子郵件|电子邮件/i] },
  { slot: "phone.full", patterns: [/phone|tel|telephone|mobile|携帯|電話|電話番号|手机|手机号|联系电话/i] },
  ...
  { slot: "date", patterns: [/date|dob|birth|birthday|年月|出生日期|生年月日|取得年月|入学年月|卒業年月/i] },
  { slot: "company", patterns: [/company|organization|organisation|employer|会社|会社名|法人|公司/i] },
  { slot: "department", patterns: [/school|university|college|高校|学校|学部|学科|学校信息/i] },
  { slot: "username", patterns: [/actor|badge name|nickname|nick.?name|用户名|昵称|アカウント|account/i] },
  { slot: "url", patterns: [/facebook|linkedin|wantedly|github|twitter|x\.com|social|sns|profile url/i] },
  { slot: "title", patterns: [/badge description|description|self.?intro|signature|签名|自己紹介|志望|理由/i] },
  { slot: "name.first", patterns: [/first name|given name|(?:^|[\s:：])名(?:$|[\s:：])/iu] },
  { slot: "name.last", patterns: [/last name|family name|surname|(?:^|[\s:：])姓(?:$|[\s:：\(（])/iu] },
...
function inferRepresentation(info: FieldInfo, slot: string): string | undefined {
  ...
  if (slot.startsWith("postal_code")) {
    if (slot.endsWith(".part1") || /(?:zip|postal|yubin).*(?:1|part1)/i.test(all)) return "split_3_4";
    if (slot.endsWith(".part2") || /(?:zip|postal|yubin).*(?:2|part2)/i.test(all)) return "split_3_4";
...
export function applyPostalAndPhoneRepresentations(fields: FieldInfo[]): void {
  for (let i = 0; i < fields.length - 1; i++) {
    ...
    if (/郵便番号|邮编|postal|zip/.test(label) && aMax === "3" && bMax === "4") {
      a.semantic = { slot: "postal_code.part1", confidence: 0.9, representation: "split_3_4", evidence: ["adjacent maxlength 3/4 postal fields"] };
      b.semantic = { slot: "postal_code.part2", confidence: 0.9, representation: "split_3_4", evidence: ["adjacent maxlength 3/4 postal fields"] };
    }
    if (/電話|phone|tel/.test(label) && (aMax === "2" || aMax === "3") && (bMax === "3" || bMax === "4")) {
      a.semantic = { slot: "phone.area_code", confidence: 0.75, representation: "split_2_or_3", evidence: ["adjacent phone maxlength pattern"] };
      b.semantic = { slot: "phone.national", confidence: 0.75, representation: "split_2_or_3", evidence: ["adjacent phone maxlength pattern"] };
    }
  }
}
